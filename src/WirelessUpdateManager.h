...
public:
  ...
  void begin() {
    ...
    server.on("/serverIndex", HTTP_GET, [this]() {
      server.sendHeader("Connection", "close");
      server.send(200, "text/html", uploadPageHtml);
    });
...
private:
  WebServer server; // Use member initializer list to initialize the WebServer
